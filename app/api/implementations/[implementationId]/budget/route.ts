...
import { approvedDays, validateBudgetRows, budgetWithRemainder } from "@/lib/implementation-planning";
...
export async function PUT(request:Request,context:Context) {
 try {
  ...
  if(body.budget!==state.budget || !validateBudgetRows(body.rows,null)) return json({error:"De som van de begrote dagen moet gelijk zijn aan het goedgekeurde dagenbudget. Vernieuw bij een gewijzigde offerte."},400);
  const current = await getImplementationBudgetState(state.implementation!.id);
  const balance = budgetWithRemainder(state.budget, body.rows, current.hoursPerDay);
  if (body.draft !== true && (!balance || balance.overBudget)) return json({error:"De begrote uren overschrijden het offertebudget. Verlaag de uren of sla op als concept."},400);

...
import {validateBudgetRows} from "@/lib/implementation-planning";
...
export async function getImplementationForecast(implementationId:string){
 const {budget,rows:allocation,approvals,ticketId,allocationComplete}=await getImplementationBudgetState(implementationId);
 let progress:number|null=null;
 if(allocationComplete&&budget!==null&&budget>0&&validateBudgetRows(allocation,budget))progress=Math.min(100,Object.entries(allocation).reduce((total,[key,row])=>total+row.days*(approvals[key]?1:row.started?0.5:0),0)/budget*100);
